...
```
+--------------------------------------------------------------+
|  未来层: LLM 自然语言查询 (NL → 可信统计回答)                  |
+--------------------------------------------------------------+
|  本层: 可信事件流数据底座 (本项目)                             |
|  HTML牌谱 → 解析 → 追踪 → 多级验证 → 纯净局准入 → parquet    |
+--------------------------------------------------------------+
```
...
```mermaid
graph TD
    A[天凤 HTML/XML 牌谱<br/>354K 局原始数据] --> B[解析层<br/>pipeline/tenhou_to_mjai.py]
    B --> C[事件流追踪层<br/>pipeline/etl_v3.py]
    C --> D[验证层<br/>verify_gate.py L0-L3]
    D --> E[规则自检<br/>现物=0 / 反事实]
    D --> F[准入层<br/>纯净局 (ADR-005)]
    E --> F
    F --> G[(v3_200k_v2.parquet<br/>可信事件流)]
    G --> H[查询族 analysis/<br/>统计结果 results/]
    G --> I[未来: LLM NL 查询层]
```
