...
│  本层: 可信事件流数据底座 (本项目)                             │
│                                                             │
│  天凤 HTML/XML 牌谱 (354K 局)                                │
│      │  ① 解析层   pipeline/tenhou_to_mjai.py               │
│      ▼                                                      │
│  MJAI 事件流                                                 │
│      │  ② 追踪层   pipeline/etl_v3.py                       │
│      ▼                                                      │
│  原始事件流                                                   │
│      │  ③ 验证层   verify_gate.py L0-L3                     │
│      ├── 规则自检 (现物=0 / 反事实) ④                         │
│      ▼                                                      │
│  v3_200k_v2.parquet (2.05M 局)   ← ⑤ 准入层 (纯净局 ADR-005) │
│      │                                                      │
│      ├── 查询族 analysis/ → results/                        │
│      └── (未来) LLM NL 查询层                                │
└────────────────────────────────────────────────────────────┘
```
